;(function (root, factory) {
	...
	var Network = module.exports = function Network(options) {
		...
		// To handle all models and collections requests override Backbone.sync and add this wrapper
		//
		// var oldSync = Backbone.sync;
		// Backbone.sync = function (method, model, options) {
		//    model.once('request', function (method, model, options) {
		//        Backbone.trigger('request', method, model, options);
		//    });
		//    return oldSync.call(model, method, model, options);
		// }
		// ...
		// model.fetch({ network: 'myRequestId' });
		Backbone.on('request', this.onBackboneRequest, this);
	};
